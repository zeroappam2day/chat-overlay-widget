...
export function useFlagSync(sendMessage: (msg: ClientMessage) => void, connected: boolean): void {
  ...
  useEffect(() => {
    ...
    sendMessage({ type: 'set-flags', flags: { outputBatching, terminalWriteMcp, conditionalAdvance, windowFocusManager } });
    sentRef.current = true;
  }, [outputBatching, terminalWriteMcp, conditionalAdvance, windowFocusManager, connected, sendMessage]);
}
