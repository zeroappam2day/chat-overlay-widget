...
export function useFlagSync(sendMessage: (msg: ClientMessage) => void, connected: boolean): void {
  ...
  useEffect(() => {
    ...
    sendMessage({ type: 'set-flags', flags: { outputBatching, terminalWriteMcp, conditionalAdvance, batchConsent } });
    sentRef.current = true;
  }, [outputBatching, terminalWriteMcp, conditionalAdvance, batchConsent, connected, sendMessage]);
}
