...
export function useFlagSync(sendMessage: (msg: ClientMessage) => void, connected: boolean): void {
  ...
  useEffect(() => {
    ...
    sendMessage({ type: 'set-flags', flags: { outputBatching, terminalWriteMcp, conditionalAdvance, clipboardAccess } });
    sentRef.current = true;
  }, [outputBatching, terminalWriteMcp, conditionalAdvance, clipboardAccess, connected, sendMessage]);
}
