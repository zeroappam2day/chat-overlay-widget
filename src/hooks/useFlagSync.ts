...
export function useFlagSync(sendMessage: (msg: ClientMessage) => void, connected: boolean): void {
  ...
  useEffect(() => {
    ...
    sendMessage({ type: 'set-flags', flags: { outputBatching, terminalWriteMcp, conditionalAdvance, workflowRecording } });
    sentRef.current = true;
  }, [outputBatching, terminalWriteMcp, conditionalAdvance, workflowRecording, connected, sendMessage]);
}
