...
server.tool('bind_annotation_to_element', `Bind an annotation to a UI element so it automatically tracks the element's position when the window moves, scrolls, or resizes.
...
Example — bind to a menu item by name and role:
  { "annotationId": "step2", "strategy": "nameRole", "name": "File", "role": "MenuItem", "hwnd": 12345 }`, {
    annotationId: zod_1.z.string().min(1).max(200).describe('ID of an existing annotation to bind'),
    strategy: zod_1.z.enum(['automationId', 'nameRole', 'coordinates']).describe('How to find the target element'),
    automationId: zod_1.z.string().optional().describe('UI Automation ID (for automationId strategy)'),
    name: zod_1.z.string().optional().describe('Element name (for nameRole strategy)'),
    role: zod_1.z.string().optional().describe('Control type role like Button, Edit, MenuItem (for nameRole strategy)'),
    hwnd: zod_1.z.number().int().describe('Window handle of the target window'),
    offsetX: zod_1.z.number().int().optional().describe('Pixel offset from element top-left X'),
    offsetY: zod_1.z.number().int().optional().describe('Pixel offset from element top-left Y'),
    ...
    try {
        ...
        if (resp.status === 403) {
            const errBody = JSON.parse(resp.body.toString('utf-8'));
            return { content: [{ type: 'text', text: `Feature flag error: ${errBody.error}` }], isError: true };
        }
        if (resp.status !== 200) {
            const errText = resp.body.toString('utf-8');
            return { content: [{ type: 'text', text: `HTTP ${resp.status}: ${errText}` }], isError: true };
        ...
        const result = JSON.parse(resp.body.toString('utf-8'));
        return { content: [{ type: 'text', text: `Annotation "${result.annotationId}" bound to element via ${result.strategy} strategy.` }] };
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        return { content: [{ type: 'text', text: msg }], isError: true };
