...
Always query Repowise MCP tools FIRST for any feature, function, location, risk, or journey question. Use get_overview(), get_context(), get_risk(), search_codebase(), get_dependency_path(), get_dead_code(), get_architecture_diagram(). Never guess code locations or assume stale docs.

**Known limitations (v0.2.1):** get_why() hangs 50% of calls — use get_context() + Decisions section instead. search_codebase() confidence scores are broken (always 1.0) — use relevance_score for ranking.
...
|------|-------|-------------|-------|
| `sidecar/src/server.ts` | 99.4th %ile | 32 | anuj datta |
| `sidecar/dist/server.js` | 98.9th %ile | 11 | anuj datta |
| `sidecar/src/mcp-server.ts` | 98.3th %ile | 9 | anuj datta |
| `src/components/TerminalPane.tsx` | 97.7th %ile | 30 | anuj datta |
| `sidecar/src/windowCapture.ts` | 97.2th %ile | 6 | anuj datta |
...
- **Windows Defender real-time scanning locks .rcgu.o files during Rust compilation, causing "os error 32"**: Requires env vars + retry wrapper (see MEMORY.md reference).
